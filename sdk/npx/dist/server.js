...
// ==================== 服务器实例 ====================
const server = new Server({ name: "mcp-npx-sdk", version: "1.0.0" }, { capabilities: { tools: { listChanged: true } } });
// 幂等性缓存
...
server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
    ...
    switch (name) {
        ...
        case "test_tool_call": {
            const input_value = args?.input_value;
            const input_type = args?.input_type || "string";
        ...
        case "test_all_types": {
            ...
            if (args?.string_value)
                results.string = { received: args.string_value, type: "string", valid: true };
            if (args?.integer_value != null)
                results.integer = { received: args.integer_value, type: "number", valid: true };
            if (args?.float_value != null)
                results.float = { received: args.float_value, type: "number", valid: true };
            if (args?.boolean_value != null)
                results.boolean = { received: args.boolean_value, type: "boolean", valid: true };
            if (args?.negative_value != null)
                results.negative = { received: args.negative_value, type: "number", valid: true };
            if (args?.big_int_value != null)
                results.bigint = { received: args.big_int_value, type: "number", valid: true };
            if (args?.array_value)
                results.array = { received: args.array_value, type: "array", valid: true };
            if (args?.object_value)
                results.object = { received: args.object_value, type: "object", valid: true };
        ...
        case "test_complex_params": {
            ...
            const enum_value = args?.enum_value || "option1";
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_long_operation": {
            const duration_seconds = args?.duration_seconds || 3;
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_error_codes": {
            const error_type = args?.error_type || "invalid_params";
            const errorMap = {
        ...
        case "test_progress_notification": {
            const steps = Math.max(1, Math.min(args?.steps || 3, 10));
        ...
        case "test_cancellation": {
            const duration_seconds = args?.duration_seconds || 2;
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_completion": {
            const partial_value = args?.partial_value || "";
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_idempotency": {
            const operation_id = args?.operation_id || "";
        ...
        case "test_timeout_boundary": {
            const duration_seconds = args?.duration_seconds || 5;
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "gui_automation_demo": {
            const scenario = args?.scenario || "notepad";
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_elicitation_form": {
            const form_title = args?.form_title || "用户信息";
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_elicitation_url": {
            const auth_url = args?.auth_url || "https://example.com/auth";
            return {
                content: [{ type: "text", text: JSON.stringify({
        ...
        case "test_sampling_basic": {
            const prompt = args?.prompt || "What is 2+2?";
            return {
                content: [{ type: "text", text: JSON.stringify({
