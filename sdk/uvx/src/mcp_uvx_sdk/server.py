"""
MCP 测试服务器 - UVX 传输 (官方 SDK, Low-Level Server API)

使用官方 mcp Python SDK 的底层 Server API 实现 stdio 传输。
...
启动方式:
  uvx --from . mcp-uvx-sdk
  或
  python -m mcp_uvx_sdk.server
"""
...
def main():
    """启动 stdio 服务器（同步入口，供 hatch 打包调用）"""
    import asyncio
    ...
    async def _run():
        async with stdio_server() as (read_stream, write_stream):
            await server.run(
                read_stream,
                write_stream,
                server.create_initialization_options(NOTIFICATION_OPTIONS),
            )
